...
def signups():
    ...
    if show_form.show.data and show_form.validate():  # проверка,есть ли у кнопки (show.data) данные для
        # отправки методом "POST"
    ...
    if signup_form.insert.data and signup_form.validate():  # проверка,есть ли у кнопки (insert.data) данные
    ...
    if form.validate_on_submit():
        try:
            ...
            msg.body = f'Новое обращение от {form.name.data}.\nТелефон для связи: {form.tel.data}.\n' \
                       f'Текст обращения: {form.text.data}!'
            mail.send(msg)
        except DBAPIError as error:
            db.session.rollback()
            print("Ошибка добавления в БД " + str(error))
