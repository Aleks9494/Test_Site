...
from app.admin.forms import LoginFormAdmin, AddPostAdmin, UpdatePostAdmin, AddCourseAdmin, AddLessonAdmin, UpdateCourseAdmin,\
...
def index():
    ...
    form = SelectTeacher()
    form.name.choices = [(teacher.id, teacher.name) for teacher in Teacher.query.all()] #создаем кортеж из id и имени для selectfield
    ...
    if form.validate_on_submit():
        ...
        #teacher = Teacher.query.filter_by(id=form.name.data).first() # получаем все данные по id
    return render_template('admin/index.html', title='Админ-панель', menu=menu, user=current_user, form=form) #с помощью user_loader
...
def addPost():
    ...
    if form.validate_on_submit():
        ...
        if file and validate_name(file.filename): # если считали, и расширение png или jpg
            ...
            try:
                result = Post(title=form.title.data, text=form.text.data, image=route, url=form.url.data) #считываем данные из формы, создаем объект класса Post
        ...
        if file and validate_name(file.filename): # если считали, и расширение png или jpg
            ...
            try:
                result = Course(title=form.title.data, duration=form.duration.data, age=form.age.data,
                                 text=form.text.data, image=route, url=form.url.data) #считываем данные из формы, создаем объект класса
                db.session.add(result) #добавляем в БД
                db.session.commit()
        ...
        if file and validate_name(file.filename):  # если считали, и расширение png или jpg
            ...
            try:
                result = Lesson (title=form.title.data, duration=form.duration.data, age=form.age.data,
                                ...
                                url=form.url.data, course_id=course.id)  # считываем данные из формы, создаем объект класса
                db.session.add(result)  # добавляем в БД
                db.session.commit()
        ...
        if file and validate_name(file.filename):  # если разрешение правильное
            ...
            try:
                result = Teacher(name=form.name.data, surname=form.surname.data, age=form.age.data,
                                 tel=form.tel.data, image=route, email=form.email.data, text=form.text.data) #считываем данные из формы, создаем объект класса
                db.session.add(result) #добавляем в БД
                db.session.commit()
