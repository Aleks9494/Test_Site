...
from wtforms import StringField, SubmitField, BooleanField, PasswordField, TextAreaField, FileField, EmailField, IntegerField, \
...
class AddTeacherAdmin(FlaskForm):
    ...
    # def validate_email(self, email): #название функциио обязательно validate_...имя поля
    #     if Teacher.query.filter_by(email=email.data).first():
    #         raise ValidationError('Почта уже занята')
    #
    # def validate_tel(self, tel):
    #     if Teacher.query.filter_by (tel=tel.data).first():
